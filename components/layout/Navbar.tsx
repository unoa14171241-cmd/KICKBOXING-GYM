            ...
            <div className="hidden md:flex items-center gap-4">
              {status === 'loading' ? (
                ...
                <div className="flex items-center gap-4">
                  {(session.user.role === 'owner' || session.user.role === 'trainer') && (
                    <Link
                      href="/admin"
                  ...
                  {session ? (
                    <>
                      {(session.user.role === 'owner' || session.user.role === 'trainer') && (
                        <Link
                          href="/admin"
                          className="flex items-center gap-2 py-2 text-primary-400 hover:text-primary-300 transition-colors font-semibold"
                          onClick={() => setIsMenuOpen(false)}
                        >
                          <LayoutDashboard className="w-5 h-5" />
                          <span>管理画面</span>
